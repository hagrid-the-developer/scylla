...
future<> distributed_loader::populate_column_family(distributed<database>& db, sstring sstdir, sstring ks, sstring cf) {
    ...
    struct sstable_descriptor {
        ...
        sstable_descriptor(status st, sstables::sstable::version_types v, sstables::sstable::format_types f)
            : _status(st)
            , version(v)
            , format(f) {
        }
        ...
        return lister::scan_dir(sstdir, { directory_entry_type::regular }, [&db, verifier, &futures] (lister::path sstdir, directory_entry de) {
            ...
            auto f = distributed_loader::probe_file(db, sstdir.native(), de.name).then([verifier, sstdir, de] (auto entry) {
                ...
                if (verifier->count(entry.generation)) {
                    if (verifier->at(entry.generation)._status == status::has_toc_file) {
                        ...
                        verifier->at(entry.generation)._status = status::has_toc_file;
                    } else if (entry.component == sstables::sstable::component_type::TemporaryTOC) {
                        verifier->at(entry.generation)._status = status::has_temporary_toc_file;
                    }
                } else {
                    if (entry.component == sstables::sstable::component_type::TOC) {
                        auto sd = sstable_descriptor{status::has_toc_file, entry.version, entry.format};
                        verifier->emplace(entry.generation, sd);
                    } else if (entry.component == sstables::sstable::component_type::TemporaryTOC) {
                        auto sd = sstable_descriptor{status::has_temporary_toc_file, entry.version, entry.format};
                        verifier->emplace(entry.generation, sd);
                    } else {
                        auto sd = sstable_descriptor{status::has_some_file, entry.version, entry.format};
                        verifier->emplace(entry.generation, sd);
                    }
                }
                return make_ready_future<>();
            ...
            return do_for_each(*verifier, [sstdir = std::move(sstdir), ks = std::move(ks), cf = std::move(cf), verifier] (auto v) {
                if (v.second._status == status::has_temporary_toc_file) {
                ...
                } else if (v.second._status != status::has_toc_file) {
