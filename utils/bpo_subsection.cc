...
class YamlParser {
public:
    YamlParser(std::unordered_map<std::string, std::vector<std::string>>& map)
        : _map{map} {
    }
...
config::bpo_subsection::bpo_subsection(const stdx::string_view& name)
    : named_value{name} {
}

void config::bpo_subsection::set_value(const YAML::Node& node) {
    YamlParser{(*this)()}.parse(node);
    source(config_source::SettingsFile);
}

bpo::parsed_options config::bpo_subsection::parsed_options(const bpo::options_description& opts) const {
