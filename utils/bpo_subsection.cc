...
bpo_subsection::bpo_subsection(const stdx::string_view& name)
    : named_value{name} {
}

void bpo_subsection::set_value(const YAML::Node& node) {
    YamlParser{(*this)()}.parse(node);
    source(config_file::config_source::SettingsFile);
}

bpo::parsed_options bpo_subsection::parsed_options(const bpo::options_description& opts) const {
