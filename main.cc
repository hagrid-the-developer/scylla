...
/*
void store_from_yaml(const bpo::parsed_options& options, bpo::variables_map& xm, bool utf8 = false) {
    // XYZ: Where do we check whether variable exists?
    // TODO: what if we have different definition
    // for the same option name during different calls
    // 'store'.
    assert(options.description);
    const bpo::options_description& desc = *options.description;

    // We need to access map's operator[], not the overriden version
    // variables_map. Ehmm.. messy.
    // XYZ: Fix this.
    std::map<std::string, bpo::variable_value>& m = xm;
    ...
    try {
        ...
        for (unsigned i = 0; i < options.options.size(); ++i) {
            ...
            // XYZ: Use this: const std::string option_name_hyphenated = utils::hyphenate(option_name);
            std::string option_name_hyphenated = option_name;
 ...
 //           if (xm.m_final.count(option_name_hyphenated))
 //               continue;

            original_token = options.options[i].original_tokens.size() ?
                                    options.options[i].original_tokens[0]     : "";
            const bpo::option_description& d = desc.find(option_name_hyphenated, false,
                                                    false, false);

            bpo::variable_value& v = m[option_name_hyphenated];
            if (v.defaulted()) {
                // Explicit assignment here erases defaulted value
                v = bpo::variable_value();
            }
